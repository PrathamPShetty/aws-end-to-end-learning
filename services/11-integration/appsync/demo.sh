...
# ============================================================
# AppSync Demo
# Purpose: Create GraphQL API, schema, DynamoDB data source, API key
# ============================================================
set -e
...
ROLE_ARN="arn:aws:iam::${ACCOUNT}:role/appsync-demo-role"
TABLE_NAME="TodoTable"

echo "==> Creating DynamoDB table as data source backend..."
...
awslocal iam create-role --role-name appsync-demo-role \
  --assume-role-policy-document '{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":{"Service":"appsync.amazonaws.com"},"Action":"sts:AssumeRole"}]}' > /dev/null 2>&1 || true
awslocal iam attach-role-policy --role-name appsync-demo-role \
...
API_ID=$(awslocal appsync create-graphql-api \
  --name "$API_NAME" --authentication-type API_KEY \
...
awslocal appsync create-api-key --api-id "$API_ID" --description "Demo key" \
...
awslocal appsync start-schema-creation --api-id "$API_ID" \
  --definition "$(printf '%s' "$SCHEMA" | base64)" > /dev/null 2>&1 || true
echo "  Schema uploaded"

echo "==> Attaching DynamoDB data source..."
awslocal appsync create-data-source --api-id "$API_ID" \
  --name "TodoDynamoDB" --type AMAZON_DYNAMODB --service-role-arn "$ROLE_ARN" \
  --dynamodb-config "tableName=${TABLE_NAME},awsRegion=us-east-1" > /dev/null 2>&1 || true

echo "==> Listing APIs..."
awslocal appsync list-graphql-apis \
  --query 'graphqlApis[].{Name:name,ApiId:apiId,AuthType:authenticationType}' --output table
